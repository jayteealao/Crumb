...
sealed interface SyncErrorEvent {
    ...
    data class TwitterAuth401(
        val message: String = "Twitter session expired",
    ) : SyncErrorEvent {
        override val source: BookmarkSource = BookmarkSource.Twitter
    }

    data class RedditAuth401(
        val message: String = "Reddit session expired",
    ) : SyncErrorEvent {
        override val source: BookmarkSource = BookmarkSource.Reddit
    }

    data class Other(
        override val source: BookmarkSource,
        val message: String,
    ) : SyncErrorEvent
}
