...
fun mapTweetDataToTweetEntity(
    tweets: List<Tweet>,
    includes: TweetIncludes,
    page: Int = 0,
): List<TweetEntities> =
    tweets.mapIndexed { _, tweet ->
        ...
        TweetEntities(
            ...
            tweetMediaEntity =
                includes.media
                    ?.filter { mediaKeys.contains(it.mediaKey) }
...
//        Timber.d("${apiErrorResponse.response.raw()}")
...
val tweetEntitiesTweetEntityLens: Lens<TweetEntities, TweetEntity> =
    Lens(
        get = { it.tweetEntity },
...
val tweetEntityOrderLens: Lens<TweetEntity, Int> =
    Lens(
        get = { it.order },
