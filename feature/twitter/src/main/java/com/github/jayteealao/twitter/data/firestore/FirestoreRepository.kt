...
class FirestoreRepository
    ...
    constructor(
        ...
        private fun tweetsCol(uid: String): CollectionReference = db.collection(USERS_ROOT).document(uid).collection(TWEETS_COLLECTION)
        ...
        private fun mediaCol(uid: String): CollectionReference = db.collection(USERS_ROOT).document(uid).collection(MEDIA_COLLECTION)

        private fun metricsCol(uid: String): CollectionReference = db.collection(USERS_ROOT).document(uid).collection(METRICS_COLLECTION)

        private fun includesCol(uid: String): CollectionReference = db.collection(USERS_ROOT).document(uid).collection(INCLUDES_COLLECTION)
         ...
         */
        suspend fun getAllTweetIds(): Set<String> =
            withContext(Dispatchers.IO) {
                val uid = requireUid()
                Timber.w(
                    "getAllTweetIds: pagination by documentId() is lex-ordered; mixed-length snowflake IDs may yield non-monotonic boundaries",
                )
                try {
         ...
         */
        suspend fun getServerBookmarkCount(): Result<Long> =
            withContext(Dispatchers.IO) {
                try {
                    val uid = requireUid()
                    val total =
                        withTimeout(AGGREGATE_COUNT_TIMEOUT_MS) {
                            tweetsCol(uid)
                                .count()
        ...
        private data class TailEnumeration(
            val docs: List<CursorDoc>,
            val reachedFloor: Boolean,
        )
            ...
            withContext(Dispatchers.IO) {
                ...
                try {
                    ...
                    Timber
                        .tag("IncrementalSync")
                        .d(
                            "tail_enumerated docs_read=$docsRead kept=${result.size} resumeFrom=$lowCreatedAt reachedFloor=$reachedFloor hops=$safetyHops",
                        )
        ...
        suspend fun fetchTweetsNotInLocal(localIds: Set<String>): List<TweetEntities> =
            withContext(Dispatchers.IO) {
                try {
        ...
        private suspend fun fetchTweetEntitiesByIds(tweetIds: List<String>): List<TweetEntities> =
            coroutineScope {
                ...
                try {
                    withTimeout(BATCH_TIMEOUT_MS) {
                        ...
                        val usersDeferred =
                            async {
                                withTimeoutOrNull(SUB_FETCH_TIMEOUT_MS) {
                                    ...
                                    Timber.w(
                                        "twitterUsersCol sub-fetch timed out after ${SUB_FETCH_TIMEOUT_MS}ms for ids=${tweetIds.take(5)}",
                                    )
                                    emptyMap()
                                }
                            }
                        ...
                        val quotedTweetsDeferred =
                            async {
                                withTimeoutOrNull(SUB_FETCH_TIMEOUT_MS) {
                                    val includesByTweet = includesDeferred.await()
                                    val quotedIds =
                                        includesByTweet.values
                                            .asSequence()
                                            .flatten()
                                    ...
                                    Timber.w(
                                        "quotedTweetsCol sub-fetch timed out after ${SUB_FETCH_TIMEOUT_MS}ms for ids=${tweetIds.take(5)}",
                                    )
                                    emptyMap()
                                }
                            }
                        ...
                        val quotedAuthorsDeferred =
                            async {
                                withTimeoutOrNull(SUB_FETCH_TIMEOUT_MS) {
                                    ...
                                    Timber.w(
                                        "quotedAuthorsCol sub-fetch timed out after ${SUB_FETCH_TIMEOUT_MS}ms for ids=${tweetIds.take(5)}",
                                    )
                                    emptyMap()
                                }
                            }
                        ...
                        val textAnnotationsDeferred =
                            async {
                                withTimeoutOrNull(SUB_FETCH_TIMEOUT_MS) {
                                    ...
                                    Timber.w(
                                        "textAnnotationsCol sub-fetch timed out after ${SUB_FETCH_TIMEOUT_MS}ms for ids=${tweetIds.take(
                                            5,
                                        )}",
                                    )
                                    emptyMap()
         ...
         */
        suspend fun fetchSingleTweetEntities(tweetId: String): TweetEntities? = fetchTweetEntitiesByIds(listOf(tweetId)).firstOrNull()
        ...
        suspend fun uploadTweet(tweetEntities: TweetEntities) =
            withContext(Dispatchers.IO) {
                try {
                    val uid = requireUid()
        ...
        suspend fun uploadTweets(tweets: List<TweetEntities>) =
            withContext(Dispatchers.IO) {
         ...
         */
        suspend fun markDeleted(tweetId: String): Unit =
            withContext(Dispatchers.IO) {
                val uid =
         ...
         */
        suspend fun cancelPendingDelete(tweetId: String): Unit =
            withContext(Dispatchers.IO) {
                val uid =
                    auth.currentUser?.uid ?: run {
